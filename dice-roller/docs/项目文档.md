...
当前目标工程已完成阶段 3：空格生成标准数字 d6；DieBody 每秒发出 `RollOutcome`；掉落重生复制当前 FaceLayout。HUD Label 显示槽位与 Label（Invalid 为 `?`）。
...
| `scripts/DiceTableApp.cs` | 空格/`roll` 生成标准数字 d6（阶段 6/7 再改回全掷）；L 锁定有效面；C 清场；F2 改 slot 6 为 sword；F3 掉落重生 |
| `scenes/main.tscn` | 3D 地板 + 相机 + 灯光 + `DiceManager` + HUD Label；无静态骰子 |
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout`（含 `Changed`）/ `FaceReader` / `DieDefinition` / `DiceSessionState` |
| `addons/dice_roller/Physics/` | `DieBody`、`DiceManager`；抛掷不读 `$"/root/main"` |
...
| `tests/DiceRoller.Tests.csproj` | xunit + GodotSharp 4.7.2；固定姿态对照原版 `dN.gd`；Session / Changed / CloneForSpawn |
