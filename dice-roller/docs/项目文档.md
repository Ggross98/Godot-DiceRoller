...
当前目标工程已完成阶段 8 第一步：无 Interface 的 `scenes/test_headless_table.tscn` 实例化插件内 `DiceTable.tscn`，经 `IDiceTable` 生成、读 `RollOutcome`、`Layout.Replace` 后读新 `Content.Id`。空白宿主项目与嵌入指南仍未做。设置窗 / Toast / Help / 原版 Theme 仍是 7b。
...
| `scripts/HeadlessTableApp.cs` | 最小宿主：`SpawnStandard("d6")`、Replace `sword`、订阅 `Rolled` |
...
| `addons/dice_roller/Physics/` | `IDiceTable`、`DiceTable`、`DieBody`、`DiceManager`（`Clear(HullKind)`、`SpawnStandard(string)`、`Rolled`）、`DiceInteractionController`、`ArenaController`；不读 `$"/root/main"` |
