...
当前目标工程已完成阶段 6（收窄：HUD 统计 + JSON，无设置窗口）：空格生成 d6；HUD 显示 count/sum/sword；F2 改面并立刻轮询；F4 写 body_color；`[`/`]` 改重力；`user://settings.json` 用 HTML 颜色。完整 Statistics 面板仍是阶段 7。
...
| `scripts/DiceTableApp.cs` | 空格生成 d6；L/C/F2/F3；滚轮缩放；HUD `count`/`sum`/`sword`；F4 写 body_color；`[`/`]` 改重力 |
| `scenes/main.tscn` | `Box`（scale 0.2）地板 50×50 + 顶 + 四面 `wall.tscn` + `ResizeTicks`；`Camera3D/ZoomTicks`；`DiceManager` + `DiceInteractions`；无静态骰子 |
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout` / `FaceReader` / `DieDefinition` / `DiceSessionState` / `DragMath` / `ArenaMath` / `NumericScore` / `SettingsCodec` |
...
| `addons/dice_roller/Presentation/` | `IFacePresenter`、`BakedNumeralPresenter`（空 Apply）、`DiceTheme`（三色共享材质） |
...
| `tests/DiceRoller.Tests.csproj` | FaceMap / Session / DragMath / ArenaMath / NumericScore / SettingsCodec |
...
| `Interface/Icons/`、`Interface/Fonts/` | 图标与字体已拷入；字体 tres 仍为 Godot 3 `DynamicFont`，阶段 7 再用 |
