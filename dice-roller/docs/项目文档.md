...
当前目标工程已完成阶段 4b：空格生成标准数字 d6；悬停拖拽甩出；空白左键 GrabArea 吸取；拖到其他骰上会抬高；放下可叠在另一颗骰上。HUD Label 显示槽位与 Label（Invalid 为 `?`）。
...
| `scripts/DiceTableApp.cs` | 空格/`roll` 生成标准数字 d6（阶段 6/7 再改回全掷）；L 锁定有效面；C 清场；F2 改 slot 6 为 sword；F3 掉落重生；**不处理鼠标** |
| `scenes/main.tscn` | 3D 地板 + 相机 + 灯光 + `DiceManager` + `DiceInteractions`（`RotateTicks`、`MinDistance`/`MaxDistance`、`GrabArea`）+ HUD Label；无静态骰子 |
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout`（含 `Changed`）/ `FaceReader` / `DieDefinition` / `DiceSessionState` / `DragMath`（含叠放高度与抬高步进） |
| `addons/dice_roller/Physics/` | `DieBody`（`InteractionState`、`FaceMap`、最低/最高点）、`DiceManager`、`DiceInteractionController`；抛掷不读 `$"/root/main"` |
...
| `addons/dice_roller/Settings/` | Autoload `SettingsStore`；默认 gravity=4、allow_locked_move=false |
| `tests/DiceRoller.Tests.csproj` | xunit + GodotSharp 4.7.2；固定姿态对照原版 `dN.gd`；Session / Changed / CloneForSpawn；`DragMath` Y 平面 / clamp 80 / 叠放高度 / 抬高步进 |
...
| `World/wall.tscn` | 已改为 `StaticBody3D` + `BoxShape3D` size `(60, 120, 20)`；尚未放进主场景（阶段 5） |
