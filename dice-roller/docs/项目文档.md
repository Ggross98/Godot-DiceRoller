...
当前目标工程已把骰子资源收进 `addons/dice_roller/assets/`；启用 `plugin.cfg` 会注册 Autoload。无 Interface 的 `scenes/test_headless_table.tscn` 经 `IDiceTable` 生成、读 `RollOutcome`、换面。用法见 [嵌入指南.md](./嵌入指南.md)。设置窗 / Toast / Help / 原版 Theme / 指令语法 **不做**。
...
| 骰子选择条、操作按钮、右侧分面统计 | `godot/Interface/`（仅左右栏；不要设置窗 / Help / Toast） |

### 2.2 明确不纳入（或列为后续）

| 项 | 原因 |
|----|------|
| 设置三页窗口、Toast、Help、原版 Theme | 演示用左右栏 HUD + `SettingsStore.Set` 即写盘已够嵌入；不复原原 UI 铬 |
| 指令语法 `4d6d1` / `addons/dice_syntax` | 原插件被 gitignore，仓库中不存在；`CommandInput.gd` 只处理焦点 |
| `addons/debug_draw` | 缺失；C# 版不注册 |
| `World/camera.gd` 自由视角 | 未挂到 `main.tscn` 的 Camera 上 |
| HTML5 导出到 GitHub Pages | 原版输出到源仓库 `docs/`；C# 版不做 |
| 多人联机 | 原项目无网络代码；设置里的 Collaboration 只打开 Discord / GitHub |
| 第二份空白宿主工程验收 | 嵌入指南 + `test_headless_table.tscn` 已覆盖用法 |
...
DieDefinition（生成原型，可共享）
  └── HullKind + 默认 FaceLayout
      Presenter 在 Spawn / DiceManager.CreatePresenter 注入，不进 Core
...
| Presenter | 可替换适配器 | 默认烘焙数字；`ImageFacePresenter` 按槽叠 Decal |
...
2. `FaceLayouts.StandardNumeric(HullKind)`：槽位 k 的 FaceContent 为 `Id="k"`、`NumericValue=k`、`Label="k"`。
...
- **HullKind 字符串** 仍可作为生成键：`d4` … `d20`（`HullKinds.Parse` / `DiceAssets.DieScene`）。这只表示壳体，不表示面上写了什么。
...
| d6 | 0.4 | `BoxShape3D` Size `(1.16, 1.16, 1.16)`（全尺寸，约等于原 extents 0.58×2） |
...
不写入磁盘。物理模式下每颗骰子约每秒发出一次 RollOutcome；`RollResolveMode.Scripted` 飞行中不发，落地对准后再发一次。C# 的 Session 按 HullKind（演示）或按 FaceContent.Id（RPG）聚合：
...
C# `SettingsStore` 实际使用的键（`Set` 即写 `user://settings.json`）：

| 键 | 用途 | 默认 |
|----|------|------|
| `gravity` | 骰子 `GravityScale` | **4**（与骰子场景一致；世界重力仍是 10） |
| `allow_locked_move` | 锁定后是否仍可平移（只锁角速度） | false |
| `invert_scroll` | 滚轮缩放方向 | false |
| `invert_resize` | Shift+滚轮改场地大小方向 | false |
| `body_color` / `number_color` / `border_color` | `DiceTheme` 共享材质 / 描边 | 未设则保持资源文件颜色；JSON 为 HTML `#rrggbb` |

原版还有 `last_version`、`use_commands`、`show_input_d*`，C# 演示 **不读**。颜色不用 GDScript `str2var("Color(...)")`。
...
flowchart LR
  ...
  Settings -->|gravity| DM
  Session -->|轮询 0.5s| Stats[DiceHud HullStats]
```
...
```
DiceRoller.Core            纯逻辑：DieFaceMap、FaceLayout、RollOutcome、RollResolveMode、Session、NumericScore
DiceRoller.Physics         RigidBody3D 骰子、生成、抛掷、场地、抓取（不解释 FaceContent）
DiceRoller.Presentation    Presenter：烘焙数字 / Decal 换面（可引用 Texture2D；Core 不引用）
DiceRoller.Settings        JSON 存储 + 把颜色应用到材质
DiceRoller.Ui              选择条与 Session 统计（Godot Control；无设置窗 / Toast）
...
```
addons/dice_roller/          可整体复制到其他 Godot C# 项目的插件形态
  plugin.cfg / plugin.gd     启用后注册 DiceSession / SettingsStore
  Core/
  Physics/
  Presentation/
  Session/
  Settings/
  Ui/
  assets/                    dice / faces / world
scenes/                      演示用主场景（应用层，不进插件）
scripts/                     仅演示入口；库代码不要放这里
docs/
```

第一期类型已全部在 `addons/dice_roller/`，**不要**再往 `scripts/` 堆库代码。`scripts/` 只放演示入口。

### 7.1 建议的对外 API（其他项目使用）

完整步骤与拷贝清单见 **[嵌入指南.md](./嵌入指南.md)**。其他游戏嵌入时面向 `IDiceTable`，不要去点 HUD。`NumericScore.Sum` 不是主结果。当前接口：
...
`CreatePresenter` 在 `DiceManager` 上（HUD 的 `SpawnStandard` 走工厂），不在 `IDiceTable`。`DieDefinition` 只有 `Hull` + `Layout`。`RollOutcome.Hull` 是 `HullKind` 枚举。`FaceContent.TextureKey` 给 Presenter 用，Core 不当 `Texture2D`。
...
- **预定结果：** `table.ResolveMode = RollResolveMode.Scripted`，再 `die.ScriptNextRoll(slot)` 或 `ScriptNextRollByContentId`；投掷是动画，对准的是槽，换面仍然生效。切回 `Physics` 即真随机。见 [嵌入指南.md](./嵌入指南.md) §5.5。

场地大小、相机属于 App / 宿主。实现：[`IDiceTable.cs`](../addons/dice_roller/Physics/IDiceTable.cs)。
...
| `Window` / `WindowContainer` / `Toast` / `CheckToggle` | **不移植** |
| `CommandInput.gd` | **不移植**（无解析器） |
| `DiceManager` | `DiceManager`（按 DieDefinition 生成） |
| `DiceInteractions` | `DiceInteractionController` |
| `main.gd` | `DiceTableApp`（Node3D）；版本 Toast 不移植 |
...
导出清单里出现过 `Help.gd` / `HelpWindow.gd` / `Settings.gd`，当前源树 **没有** 这些文件。C# 演示 **不移植** 设置窗、Toast、Help、指令栏。
...
|------|----------------|
| `add_die` | N |
| `add_max` | M（Input Map 有；演示 HUD 未接） |
| `roll` | Space、R |
...
| 文件 | 状态 |
|------|------|
| `project.godot` | Godot 4.7、Jolt、主场景 `res://scenes/main.tscn`、窗口 1440×900、物理层 Box/Dice、重力 10、自定义 Input Map；启用插件 `addons/dice_roller/plugin.cfg`；Autoload `DiceSession`、`SettingsStore` |
...
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout` / `FaceReader` / `DieDefinition` / `DiceSessionState` / `DragMath` / `ArenaMath` / `NumericScore` / `SettingsCodec` / `SessionStats` / `HullKinds` / `RollResolveMode` / `ScriptedTargets` |
...
| `addons/dice_roller/plugin.cfg` | EditorPlugin + `plugin.gd`：启用时注册 / 关闭时移除两个 Autoload |
...
| `tests/DiceRoller.Tests.csproj` | FaceMap / Session / DragMath / ArenaMath / NumericScore / SettingsCodec / SessionStats / HullKinds / ScriptedTargets |
| `Interface/Icons/`、`Interface/Fonts/` | HUD 用 `Interface/Icons/`；字体 tres 仍是 Godot 3 `DynamicFont`，不迁原版 Theme |
...
指令语法、设置窗、Toast、Help、原版 Theme、HTML 导出、自由相机、空白宿主工程 **不是** 成功标准，也不再排期。
