...
当前目标工程已完成阶段 4（收窄：拖拽+锁定，无叠放/吸取）：空格生成标准数字 d6；悬停改光标、Y 平面拖拽、甩出上限 80、双击/W 抛、右键/S 锁定与放下。HUD Label 显示槽位与 Label（Invalid 为 `?`）。
...
| `scenes/main.tscn` | 3D 地板 + 相机 + 灯光 + `DiceManager` + `DiceInteractions` + `RotateTicks` + HUD Label；无静态骰子、无 GrabArea |
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout`（含 `Changed`）/ `FaceReader` / `DieDefinition` / `DiceSessionState` / `DragMath` |
| `addons/dice_roller/Physics/` | `DieBody`（`InteractionState`、`FaceMap`、最低点）、`DiceManager`、`DiceInteractionController`；抛掷不读 `$"/root/main"` |
...
| `tests/DiceRoller.Tests.csproj` | xunit + GodotSharp 4.7.2；固定姿态对照原版 `dN.gd`；Session / Changed / CloneForSpawn；`DragMath` Y 平面 / clamp 80 / d6 已朝上 |
