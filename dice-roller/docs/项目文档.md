...
当前目标工程已完成阶段 7（收窄：左右栏 + 快捷键）：左侧加骰/清/掷/锁，右侧按 `HullKind` 的 Session 统计（0.5s）；空格 / R 为 `RollAll`；数字键 1–9、N、C、L 由 `DiceHud` 处理。设置窗 / Toast / Help / 原版 Theme 仍是 7b。`user://settings.json` 仍在 `Set` 时写盘。
...
| `scripts/DiceTableApp.cs` | 滚轮缩放；F2 sword+PollNow；F3 掉落；F4 改色；`[`/`]` 改重力。空格/L/C **不**在此处理 |
| `scenes/main.tscn` | `Box`（scale 0.2）地板 50×50 + 顶 + 四面 `wall.tscn`；`DiceManager` + `DiceInteractions`；实例 `DiceHud.tscn` |
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout` / `FaceReader` / `DieDefinition` / `DiceSessionState` / `DragMath` / `ArenaMath` / `NumericScore` / `SettingsCodec` / `SessionStats` |
| `addons/dice_roller/Physics/` | `DieBody`、`DiceManager`（`Clear(HullKind)`、`WakeAll`、生成半尺寸×AreaScale）、`DiceInteractionController`（墙体 exception）、`ArenaController`；不读 `$"/root/main"` |
...
| `tests/DiceRoller.Tests.csproj` | FaceMap / Session / DragMath / ArenaMath / NumericScore / SettingsCodec / SessionStats |
