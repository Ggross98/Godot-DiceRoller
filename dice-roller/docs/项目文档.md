...
当前目标工程已完成阶段 5：空格生成 d6；拖拽/吸取/叠放；`Box` 四面墙与顶；滚轮推拉相机（距离 4–60）；Shift+滚轮改场地（0.15–1.4）并唤醒骰子。HUD Label 显示槽位与 Label（Invalid 为 `?`）。
...
| 各面自定义图片 | Presenter 适配器（分面贴图 / 图集 / Decal） | 接口存在即可，默认仍用原版烘焙数字网格 |
...
其他游戏嵌入时，应只依赖类似下面的入口，而不是去点 UI 按钮。`Sum` 不是主结果。
...
| 文件 | 状态 |
|------|------|
| `project.godot` | Godot 4.7、Jolt、主场景 `res://scenes/main.tscn`、窗口 1440×900、物理层 Box/Dice、重力 10、自定义 Input Map；Autoload `DiceSession`、`SettingsStore`（内存默认，无 JSON） |
...
| `scripts/DiceTableApp.cs` | 空格/`roll` 生成标准数字 d6（阶段 6/7 再改回全掷）；L 锁定有效面；C 清场；F2 改 slot 6 为 sword；F3 掉落重生；滚轮缩放、Shift/Alt+滚轮改场地；`+/-` 按住推拉相机 |
| `scenes/main.tscn` | `Box`（scale 0.2）地板 50×50 + 顶 + 四面 `wall.tscn` + `ResizeTicks`；`Camera3D/ZoomTicks`；`DiceManager` + `DiceInteractions`；无静态骰子 |
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout` / `FaceReader` / `DieDefinition` / `DiceSessionState` / `DragMath` / `ArenaMath` |
| `addons/dice_roller/Physics/` | `DieBody`、`DiceManager`（`WakeAll`、生成半尺寸×AreaScale）、`DiceInteractionController`（墙体 exception）、`ArenaController`；不读 `$"/root/main"` |
| `addons/dice_roller/Presentation/` | `IFacePresenter`、`BakedNumeralPresenter`（空 Apply） |
| `addons/dice_roller/Session/` | Autoload `DiceSession` 包装 `DiceSessionState` |
| `addons/dice_roller/Settings/` | Autoload `SettingsStore`；默认 gravity=4、allow_locked_move=false、invert_scroll/invert_resize=false |
| `tests/DiceRoller.Tests.csproj` | xunit + GodotSharp 4.7.2；FaceMap / Session / DragMath / ArenaMath（场地步进、相机 4–60、生成半尺寸） |
| `dice/d4.tscn` … `d20.tscn` | 根节点挂 `DieBody`；d6 碰撞 `BoxShape3D` size `(1.16, 1.16, 1.16)` |
| `dice/gltf/`、`dice/materials/` | 网格、`StandardMaterial3D`、描边/锁定着色器（`.gdshader`） |
| `Interface/Icons/`、`Interface/Fonts/` | 图标与字体已拷入；字体 tres 仍为 Godot 3 `DynamicFont`，阶段 6 再用 |
| `World/wall.tscn` | `StaticBody3D` + `BoxShape3D` size `(60, 120, 20)`；已实例化进 `Box/OtherBoxSides` |
