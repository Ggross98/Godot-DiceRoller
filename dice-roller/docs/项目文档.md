...
当前目标工程已完成阶段 2：`addons/dice_roller/Core/` 提供朝上槽位判定与 FaceLayout；`dotnet test tests/DiceRoller.Tests.csproj` 对照原版采样表。主场景仍是无脚本 d6 落地，尚未挂 DieBody / Manager。
...
| 文件 | 状态 |
|------|------|
| `project.godot` | Godot 4.7、Jolt、主场景 `res://scenes/main.tscn`、窗口 1440×900、物理层 Box/Dice、重力 10、自定义 Input Map 已登记；无 Autoload |
...
| `scripts/DiceTableApp.cs` | 空的 `Node3D` 根脚本（阶段 0 挂点） |
| `scenes/main.tscn` | 3D 地板 + 相机 + 灯光；实例化 `res://dice/d6.tscn` 于 `(0, 3, 0)` |
| `addons/dice_roller/Core/` | `DieFaceMap` / `FaceLayout` / `FaceReader`；判定返回槽位而非点数；尚未挂到刚体 |
| `tests/DiceRoller.Tests.csproj` | xunit + GodotSharp 4.7.2；固定姿态对照原版 `dN.gd` |
| `dice/d4.tscn` … `d20.tscn` | Godot 4 刚体场景（无 GDScript）；d6 碰撞 `BoxShape3D` size `(1.16, 1.16, 1.16)` |
